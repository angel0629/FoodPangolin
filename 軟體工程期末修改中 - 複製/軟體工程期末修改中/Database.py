...
def r_getallList(my_id):
	try:
		...
		print("Error connecting to DB")
		exit(1)
	sql="""SELECT orders.o_id, client_account.ca_FName,delivery_staff.d_name, orders.pickup_time, orders.delivery_time, orders.o_status, status.status_name,client_orderlist.col_Num,client_orderlist.col_Id
...
def C_addcar(Rname, Dname, Dprice, Num, Sum):
	...
	sql2="INSERT INTO `orders` (`o_id`, `r_id`, `m_id`, `c_id`, `o_status`, `d_sid`, `pickup_time`, `delivery_time`, `col_Id`) VALUES (NULL, %s, %s, %s, %s, NULL, NULL, NULL, %s)"
	
	cursor.execute(sql2, (Rid, Mid, Cid, 1, col_id))
	conn.commit()
	'''
...
#下單
def C_addorder(Rname, Rid, Mid, Cid):
...
def r_getorderinfo(col_id):
	sql="SELECT col_RName Rname, col_DName Dname, col_DPrice Dprice, col_Num num, col_Sum sum FROM client_orderlist WHERE client_orderlist.col_Id=%s"
