...
def c_homepage():
    ...
    my_cid=my_id_dict["ca_Id"]
    fname=C_getfname(my_cid)['fname']
    if not my_cid:  # 如果 my_id 是 None，表示查詢失敗
        return "查無此帳號，請重新登入", 403  # 回應 403 Forbidden 或引導至登入
    data=C_gethome()
	
    return render_template('c_homepage.html', data=data, fname=fname)
...
def home2():
    ...
    bestsell_and_num = find_bestsell_and_num(my_id)
    data_best_sell_dict={}
    for j in range(len(bestsell_and_num)):
        data_Dname=bestsell_and_num[j]['col_DName'].split(",")
        data_Num=bestsell_and_num[j]['col_Num'].split(",")
        for i in range(len(data_Dname)):
            if (data_Dname[i].strip() not in data_best_sell_dict ):
                # data_best_sell_dict.append({
                # 'Dname': data_Dname[i].strip(),
                # 'num': int(data_Num[i].strip())})
                data_best_sell_dict[data_Dname[i].strip()]=int(data_Num[i].strip())
            else:
                data_best_sell_dict[data_Dname[i].strip()]+=int(data_Num[i].strip())
        # 按數量排序，生成一個排序後的清單
    sorted_data = sorted(
        data_best_sell_dict.items(),  # 將字典轉換為 (key, value) 的 tuple 清單
        key=lambda x: x[1],  # 根據 value 進行排序
        reverse=True  # 降序
    )

    # 將排序結果轉換為格式化清單或字典（可選）
    sorted_result = [{'Dname': k, 'num': v} for k, v in sorted_data]
    ...
    return render_template('home.html', data=get_order,acc=get_r_id(account),data2=total_sales,data3=total_orders,data4=finished_order,data5=sorted_result,data6=find_comment)
...
def restaurant_sell_list():
    ...
    return render_template('r_sell_list.html', data=get_order,acc=get_r_id(account), restaurant_name=r_name["r_name"])#,data5=bestsell_and_num
...
def register_customer():
    if request.method == 'POST':
        ...
        add_customer_to_db(acc, pwd, fname, lname, telephone, email, address)
...
def restaurant_sales():
    ...
    return render_template('r_salessummary.html',acc=get_r_id(account),data5=sorted_result)
...
def r_money():
    ...
    return render_template('r_money.html',acc=get_r_id(account),data=find_restaurant_info,data2 = total_orders,data3=total_sales)

@app.route('/update_customer/<int:ca_Id>', methods=['POST'])
def c_update(ca_Id):  # 接收從 URL 傳遞過來的 r_id
    if 'loginAcc' not in session:
        return redirect('/login_or_register')  # 未登入時重定向到登入頁面
    
    form = request.form
    caFName = form['ca_FName']
    caLName = form['ca_LName']
    caEmail = form['ca_Email']
    caTel = form['ca_Tel']
    caAdd = form['ca_Add']
    update_customer_info(caFName,caLName,caEmail,caTel,caAdd,ca_Id)

    return redirect('/c_manage')#下面
...
def r_manage():
    ...
    account = session['loginAcc']
    get_ca_Id = get_ca_id(account)
    caId=get_ca_Id["ca_Id"]#字典轉變數

    if not caId: 
        return "查無此帳號，請重新登入", 

    find_c_info = get_customer_info(caId)
    return render_template('c_manage.html',acc=get_ca_id(account),data=find_c_info)
...
@app.route('/update_delivery/<int:d_sid>', methods=['POST'])
def d_update(d_sid):  # 接收從 URL 傳遞過來的 r_id
    if 'loginAcc' not in session:
        return redirect('/login_or_register')  # 未登入時重定向到登入頁面
    ...
    # 呼叫更新商家資料的函數
    update_delivery_info(dname, dphone, dcity,dcar,dgmail,d_sid)

    return redirect('/d_manage')#下面
...
def d_manage():
    ...
    dsid=get_d_sid["d_sid"]#字典轉變數

    if not dsid:  # 如果 my_id 是 None，表示查詢失敗
        return "查無此帳號，請重新登入", 403  # 回應 403 Forbidden 或引導至登入

    find_d_info = find_d_information(dsid)
    return render_template('d_manage.html',acc=get_dsid(account),data=find_d_info)
...
def r_get_food():
    ...
    dat=r_getList(my_id)
    r_name=r_get_r_name(my_id)
    return render_template('r_sell_food.html',data=dat,id_in=my_id, restaurant_name=r_name["r_name"])
...
def c_feedback(Rname, Oid):
    ...
    my_cid=my_id_dict["ca_Id"]
    fname=C_getfname(my_cid)['fname']
    return render_template('c_feedbackUPL.html', Rname=Rname, fname=fname)
...
def c_insertfb(Rname):
    ...
    my_id_dict= get_customer_cart_from_sql(account)
    my_cid=my_id_dict["ca_Id"]
    C_insertfb(my_cid, rid, rating, comment)
    data=C_gethome()
    return render_template('c_homepage.html', data=data)
...
def d_get_order(o_id, d_sid): #訂單已取
    ...
    get_pickup_time(pickup_time, d_sid, o_id)
...
@app.route('/status_to5/<int:o_id>/<int:d_sid>') #完成訂單
@login_confirm_acc_and_pwd #上鎖
def d_finish(o_id, d_sid): 
    d_change_staff_status2(d_sid)
    d_change_order_statusto5(o_id)
    account = session['loginAcc']
    staff_data = get_d_id(account)
    get_order = d_find_finished_order(d_sid) #找狀態為5 
    # r_id = get_order["r_id"]
    # restaurant = d_rinformation(r_id)
    # c_id = get_order["c_id"]
    # customer = d_cinformation(c_id)
    # delivery_time = datetime.now()
    # get_delivery_time(delivery_time, d_sid, o_id)
    return render_template('finish.html', data=get_order,  staff=staff_data )

# @app.route('/finish/<int:o_id>/delivery/finish/<int:d_sid>') #抵達客戶地址 完成定單
# @login_confirm_acc_and_pwd #上鎖
# def d_finish(o_id, d_sid): 
...
#     return render_template('track.html', data=get_order, staff=staff_data )
...
def delivery_other_info():
    ...
    # get_order = d_getrecord(getdsid)

    order = order_info(getdsid)  #收入金額
    total_income = sum(o["income"] for o in order if o["income"] is not None)
    goodrate = count_delivery_goodrate(getdsid)  
    # total_order = total_order_info(getdsid)
    return render_template('deliverhome.html', data=goodrate,orders=order,total_income=total_income)



@app.route('/track_order_status/<int:d_sid>') #訂單選項，取得我目前接單資訊
def track_order_status(d_sid):
    ...
    # 查詢該 d_sid 接的訂單
    orders = d_get_order_status(d_sid)  
    if not orders:
